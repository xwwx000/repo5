...
<script type="text/javascript">
	...
	function createRecord() {
		layer.open({
			...
			fix : false, //不固定
			maxmin : true,
			content : '${ctx}/order/addsgoodsdeptpage'
		});
	}
	//编辑
	function modifyRecord(id,deptname,deptmark,status) {
		layer.open({
			type : 2,
			area : [ '650px', '250px' ],
			fix : false, //不固定
			maxmin : true,
			content : '${ctx}/order/editsgoodsdeptpage?id='+id+'&deptname='+deptname+'&deptmark='+deptmark+'&status='+status
		});
	}	
	//删除
	...
	function ajax_delete(id){
		$.ajax({
			url : "${ctx}/order/deletesgoodsdept",
			async : true,
			type : "post",
			dataType : "json",
			data : {id:id},
...
<BODY>
	<FORM action="${ctx}/order/sgoodsdeptlist" id="sgoodsdeptlist" name="sgoodsdeptlist"
		method="post">
		<INPUT type="hidden" name="pageno" value="${pageList.pages.cpage}">
		<table width="98%" border="0" cellspacing="0" cellpadding="0"
			align="center">
			<tr>
				<td width="150" height="35" align="left" id="Title">系统维护--发货单位</td>
				<td bgcolor="#f7f7f7">
		...
		<table width='98%' border='0' id='maintbl' cellspacing='1'
			...
			<c:forEach items="${pageList.objectList}" var="list"
				...
				<TR class="defaultBGColor" align="center"
					...
					<td height="20">${list.deptname}</td>
					<td height="20">${list.deptmark}</td>
					<td><c:choose>
					...
					<td>
					<a href="#" onclick="modifyRecord('${list.id}','${list.deptname}','${list.deptmark}','${list.status}')"><span>修改</span></a>|<a href="#" onclick="deleteRecord('${list.id}')"><span>删除</span></a>
					</td>
				</tr>
			</c:forEach>			
		</TABLE>
