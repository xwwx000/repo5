...
<script type="text/javascript">
	$(function() {
		...
		$("#export").click(function() {
			...
			var stime = $("#stime").val();
			var etime = $("#etime").val();
			var url = "${ctx}/export/orderlist?stime="+stime+"&etime="+etime+"&sgoodsdept="+sgoodsdept+"&rgoodsdept="+rgoodsdept+"&goodstype="+goodstype+"&status=1";
			//alert(url);
			location.href = url;
		});
	});
...
<BODY>
	<FORM action="${ctx}/order/orderOmitList" id="orderomitlist" name="orderomitlist"
		method="post">
		<INPUT type="hidden" id="pageno" name="pageno" value="${pageList.pages.cpage}">
		<table width="98%" border="0" cellspacing="0" cellpadding="0"
			align="center">
			<tr>
				<td width="150" height="35" align="left" id="Title">运输数据--遗漏查询</td>
				<td bgcolor="#f7f7f7">
					<table width="100%" border="0" cellspacing="0" cellpadding="0"
						...
						<tr>
							...
							</select>&nbsp;&nbsp; 车牌号：<input id="carnum" name="carnum" value="${carnum}" size=10 maxlength="20"/> 
							&nbsp;&nbsp;收货单位：<input id="rgoodsdept" name="rgoodsdept"
								value="${rgoodsdept}" size=10 maxlength="60" />
							&nbsp;&nbsp; 查询日期： <input type="text" id="stime" name="stime"
								class="runcode" maxlength="10" value="${stime}" readonly />-- <input
								type="text" id="etime" name="etime" class="runcode"
								maxlength="10" value="${etime}" readonly /> <a href="#"
								id="search" class="button icon search"
								style="margin-right: 0px">查询</a><a href="#"
								id="export" class="button icon arrowdown"
