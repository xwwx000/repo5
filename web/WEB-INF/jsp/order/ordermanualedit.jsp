	...
	<input type="hidden" id="id" name="id" value="${order.id}" />
		...
		<table width="98%" border="0" cellspacing="1" cellpadding="3"
			...
			<tr class="defaultBGColor">
				...
				<td width="30%">
					<input name="consignor" id="consignor" type="text" size="20" maxlength="30" value=${order.consignor} />
				</td>
			...
			<tr class="defaultBGColor">
				...
				<select id="rgoodsdept" name="rgoodsdept">
					...
					<c:choose>
					<c:when test="${list.deptname == order.rgoodsdept}">
						<option value="${list.deptname}" selected>${list.deptname}</option>
					</c:when>
					<c:otherwise>
						<option value="${list.deptname}">${list.deptname}</option>
				...
				<td width="30%"><input type="text" id="etime" name="etime" class="runcode"
								readonly value="${order.rgoodstime}" /></td>
			</tr>
			<tr class="defaultBGColor">
				<td class="editLabel" width="20%" align="center">收货人</td>
				<td width="30%">
					<input name="consignee" id="consignee" type="text" size="20" maxlength="30" value=${order.consignee} />
				</td>
				<td class="editLabel" width="20%" align="center"></td>
				<td width="30%"><span class="contentclass"></span></td>
			</tr>	
...
<script type="text/javascript">
	...
	function save() {
		...
		if(!Number(goodsweight)){
			layer.msg('重量格式错误!');
			return;
		}
		$.ajax({
			url : "${ctx}/order/editManualOrder",
			async : true,
			type : "post",
			dataType : "json",
			data : {
