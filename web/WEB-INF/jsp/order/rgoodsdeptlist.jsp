...
<script type="text/javascript">
	...
	function createRecord() {
		layer.open({
			...
			fix : false, //不固定
			maxmin : true,
			content : '${ctx}/order/addrgoodsdeptpage'
		});
	}
	//编辑
	function modifyRecord(id,deptname,status) {
		layer.open({
			type : 2,
			area : [ '650px', '250px' ],
			fix : false, //不固定
			maxmin : true,
			content : '${ctx}/order/editrgoodsdeptpage?id='+id+'&deptname='+deptname+'&status='+status
		});
	}	
	//删除
	...
	function ajax_delete(id){
		$.ajax({
			url : "${ctx}/order/deletergoodsdept",
			async : true,
			type : "post",
			dataType : "json",
			data : {id:id},
...
<BODY>
	<FORM action="${ctx}/order/rgoodsdeptlist" id="rgoodsdeptlist" name="rgoodsdeptlist"
		method="post">
		<INPUT type="hidden" name="pageno" value="${pageList.pages.cpage}">
		<table width="98%" border="0" cellspacing="0" cellpadding="0"
			align="center">
			<tr>
				<td width="150" height="35" align="left" id="Title">系统维护--收货单位</td>
				<td bgcolor="#f7f7f7">
		...
		<table width='98%' border='0' id='maintbl' cellspacing='1'
			...
			<c:forEach items="${pageList.objectList}" var="list"
				...
				<TR class="defaultBGColor" align="center"
					...
					<td>
					<a href="#" onclick="modifyRecord('${list.id}','${list.deptname}','${list.status}')"><span>修改</span></a>|<a href="#" onclick="deleteRecord('${list.id}')"><span>删除</span></a>
					</td>
				</tr>
			</c:forEach>			
		</TABLE>
